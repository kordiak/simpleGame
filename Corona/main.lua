...
functions.startGame = function ()
    ...
    functions.enemyCreator = function ()
        for i = 1, (properties.enemiesNumber+ math.round(properties.currentLevel/2)) do
             local q = true
            while q do
...
functions.enemyMove = function()
    ...
    for i =1 , #enemiesTable do
        --TODO enemy movement
      --  if enemiesTable[i].type == "melee" then   ---- Checking what type of enemy it is and what would be his behavior
            --- MELEE IS TRYING TO GO TOWARDS YOU!!! ---
            for j =1, #mainBoard[enemiesTable[i].boardPositiong].coherentHexes do
                local a =  mainBoard[enemiesTable[i].boardPositiong].coherentHexes[j]
              print ("Board Position",  mainBoard[enemiesTable[i].boardPositiong].coherentHexes[j])
        ...
        if miniDistanceEnemyAndHero.hexNumber then
          ...
          --  enemiesTable[i].y =  mainBoard[miniDistanceEnemyAndHero.hexNumber].y
            ...
            mainBoard[enemiesTable[i].boardPositiong].isFree = true
            mainBoard[enemiesTable[i].boardPositiong].isWalkAble = true
            mainBoard[miniDistanceEnemyAndHero.hexNumber].content = mainBoard[enemiesTable[i].boardPositiong].content
            mainBoard[enemiesTable[i].boardPositiong].content = nil
            enemiesTable[i].boardPositiong = miniDistanceEnemyAndHero.hexNumber
            mainBoard[miniDistanceEnemyAndHero.hexNumber].isFree = false
            mainBoard[enemiesTable[i].boardPositiong].isWalkAble = false

            transition.to( enemiesTable[i], { time=properties.enemyTransTime, x=mainBoard[enemiesTable[i].boardPositiong].x, y= mainBoard[enemiesTable[i].boardPositiong].y, onComplete= functions.enemyTransCompleted } )
